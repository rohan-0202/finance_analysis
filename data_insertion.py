...
@retry_on_rate_limit()
def save_historical_data(ticker_symbol, period="2y", interval="1d", db_name='stock_data.db'):
    ...
    try:
        ...
        conn.commit()
        conn.close()
        print(f"Historical data for {ticker_symbol} ({period}, {interval}) saved to database.")
    except Exception as e:
    ...
    try:
        ...
        if not income_stmt.empty and not balance_sheet.empty:
            for date in income_stmt.columns:
                ...
                balance_date = min(balance_sheet.columns, key=lambda x: abs((x - date).days)) if balance_sheet.columns.size > 0 else None
                
                if balance_date is not None:
                    total_equity = balance_sheet.loc['Total Stockholder Equity', balance_date] if 'Total Stockholder Equity' in balance_sheet.index else None
                    total_debt = balance_sheet.loc['Total Debt', balance_date] if 'Total Debt' in balance_sheet.index else None
        ...
        if not quarterly_income.empty and not quarterly_balance.empty:
            for date in quarterly_income.columns:
                ...
                # Find closest balance sheet date
                balance_date = min(quarterly_balance.columns, key=lambda x: abs((x - date).days)) if quarterly_balance.columns.size > 0 else None
                
                if balance_date is not None:
                    total_equity = quarterly_balance.loc['Total Stockholder Equity', balance_date] if 'Total Stockholder Equity' in quarterly_balance.index else None
                    total_debt = quarterly_balance.loc['Total Debt', balance_date] if 'Total Debt' in quarterly_balance.index else None
                    debt_to_equity = (total_debt / total_equity) if (total_equity and total_debt) else None
                else:
                    debt_to_equity = None
                ...
                operating_income = quarterly_income.loc['Operating Income', date] if 'Operating Income' in quarterly_income.index else None
                operating_margin = (operating_income / revenue) if (operating_income and revenue) else None
...
def save_options_data(ticker_symbol, db_name='stock_data.db'):
    ...
    try:
        ...
        dates = [date for date in all_dates if datetime.strptime(date, '%Y-%m-%d').timestamp() <= six_months_later]
...
def save_recent_news(ticker_symbol, db_name='stock_data.db'):
    ...
    try:
        ...
        recent_news = [item for item in news if datetime.fromtimestamp(item.get('providerPublishTime', 0)) >= recent_cutoff]
...
def save_all_data_for_ticker(ticker_symbol, db_name='stock_data.db'):
    ...
    try:
        ...
        print(f"All essential trading data for {ticker_symbol} has been saved to {db_name}.")
        return True
    except Exception as e:
...
if __name__ == "__main__":
    ...
    limit = input(f"Found {len(tickers)} tickers. How many would you like to process? (Enter a number or 'all'): ")
    ...
    delay_input = input("Enter delay in seconds between processing tickers (recommended: 1-5, default: 2): ")
    try:
    ...
    if resume_option == 'y':
        ...
        if resume_ticker in tickers:
            ...
            print(f"Resuming from {resume_ticker} (index {start_idx+1}/{len(tickers)})")
        else:
            print(f"Ticker {resume_ticker} not found. Starting from the beginning.")
    
    print(f"Processing {len(tickers)-start_idx} tickers with {delay} seconds delay between tickers...")
