...
class TickerHistoryAnalyzer:
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                for analysis in analyses:
                    ...
                    analysis["risk_factors"] = [row["factor"] for row in cursor.fetchall()]
                    ...
                    analysis["catalysts"] = [row["catalyst"] for row in cursor.fetchall()]
                
                logger.info(f"Found {len(analyses)} analyses for {ticker} in specified timeframe")
                return analyses
    ...
    def get_ticker_news(
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                logger.info(f"Found {len(news)} news articles for {ticker} in specified timeframe")
                return news
            ...
            with sqlite3.connect(self.db_path) as conn:
                ...
                for row in cursor.fetchall():
                    ...
                    try:
                        ...
                        analyzed_at = datetime.fromisoformat(row[0].replace('Z', '+00:00'))
    ...
    def analyze_sentiment_distribution(
        ...
        sentiment_distribution = {
            'bullish': f"{sentiment_counts['bullish']} ({sentiment_counts['bullish']*100/total_analyses:.1f}%)" if total_analyses > 0 else "0 (0%)",
            'neutral': f"{sentiment_counts['neutral']} ({sentiment_counts['neutral']*100/total_analyses:.1f}%)" if total_analyses > 0 else "0 (0%)",
            'bearish': f"{sentiment_counts['bearish']} ({sentiment_counts['bearish']*100/total_analyses:.1f}%)" if total_analyses > 0 else "0 (0%)"
        }
    ...
    def display_ticker_report(
        ...
        for analysis in analyses[-3:]:
            analysis_date = datetime.fromisoformat(analysis['analyzed_at'].replace('Z', '+00:00')).strftime('%Y-%m-%d')
            print(f"  {analysis_date} - Sentiment: {analysis['sentiment']}, Confidence: {analysis['confidence']}")
        ...
        for article in news[:5]:
            if 'publish_date' in article and article['publish_date']:
                try:
                    pub_date = datetime.fromisoformat(article['publish_date'].replace('Z', '+00:00')).strftime('%Y-%m-%d')
                except ValueError:
...
@click.option(
    ...
    default=10,
    type=int,
    help="Maximum number of news articles to display"
)
@click.option(
    "--db-path", 
    default=os.getenv("DB_PATH", "finance_news.db"),
