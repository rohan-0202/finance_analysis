...
class Backtest:
    ...
    def _prepare_data(
        ...
        for ticker in tickers:
            try:
                ...
                if ticker_data.index[0] > buffer_start_date_utc:
                    print(f"Warning: Data for {ticker} only starts at {ticker_data.index[0].date()}, which is after the buffer start date {buffer_start_date.date()}.")
        ...
        combined_data.index.names = ["Timestamp", "Ticker"]
    ...
    def _prepare_benchmark_data(self, benchmark_ticker: str, all_data: pd.DataFrame) -> pd.Series:
        """
    ...
    def run(
        ...
        for i, current_timestamp in enumerate(backtest_timestamps):
            ...
            if i % 100 == 0:
                print(f"Processing day {i+1}/{len(backtest_timestamps)}: {current_timestamp.date()}")
                
            # Provide data up to and including the current timestamp
            data_slice = all_data[all_data.index.get_level_values("Timestamp") <= current_timestamp]
            ...
            if not data_slice.empty:
                ...
                self.portfolio.update_prices(self.portfolio.current_prices, current_timestamp)
        ...
        if not self.portfolio.holdings:
            ...
            for ticker, shares in self.portfolio.holdings.items():
                if shares > 0:
                    ...
                    print(f"  {ticker}: {shares} shares @ ${current_price:,.2f} (Value: ${value:,.2f})")
        ...
        if hasattr(self.portfolio, 'trade_history') and self.portfolio.trade_history:
            ...
            if len(trades_df) > 0:
                ...
                for _, trade in recent_trades.iterrows():
                    ...
                    print(f"  {ts} - {direction} {quantity} {ticker} @ ${price:.2f} (Value: ${value:.2f})")
...
if __name__ == "__main__":
    ...
    @click.command()
    @click.option("--strategy", required=True, help="Strategy file to use (e.g., 'rsi_strategy.py')")
    @click.option("--ticker", default="SPY", help="Ticker symbol to backtest")
    @click.option("--db-name", default="stock_data.db", help="Database name for historical data")
    @click.option("--months", type=int, default=12, help="Number of months to backtest")
    @click.option("--start-cash", type=float, default=10000.0, help="Initial capital for the portfolio")
    @click.option("--commission", type=float, default=0.001, help="Commission per trade (as a percentage)")
    ...
    def run_backtest(strategy, ticker, db_name, months, start_cash, commission, plot):
        ...
        try:
            ...
            if not os.path.exists(strategies_dir):
                ...
                strategies_dir = os.path.join(os.getcwd(), 'src', 'backtesting', 'strategies')
            if not os.path.exists(strategies_dir):
            ...
            if not os.path.exists(strategy_path):
                print(f"Error: Strategy file '{strategy}' not found in '{strategies_dir}'")
                return
        ...
        print(f"Running {strategy_class.__name__} for {ticker} from {start_date.date()} to {end_date.date()}")
