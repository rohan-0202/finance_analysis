...
class StochasticMeanReversionStrategy(Strategy):
    ...
    def __init__(self, portfolio: Portfolio):
        ...
        self.stop_loss_manager = StopLossManager(
            portfolio, self.parameters["stop_loss_parameters"]
        )
        # Track entry prices for profit targets (not managed by StopLossManager)
        self.entry_prices = {}
    ...
    def set_parameters(self, **kwargs):
        ...
        if "trailing_profit_trigger" in kwargs:
            self.parameters["trailing_profit_trigger"] = kwargs[
                "trailing_profit_trigger"
            ]
        if "trailing_profit_distance" in kwargs:
            self.parameters["trailing_profit_distance"] = kwargs[
                "trailing_profit_distance"
            ]
        if "stop_loss_parameters" in kwargs:
            self.parameters["stop_loss_parameters"] = kwargs["stop_loss_parameters"]

        # Update stop loss manager parameters
        self.stop_loss_manager.parameters = self.parameters["stop_loss_parameters"]
    ...
    def generate_signals(self, data: DataDict) -> Dict[str, float]:
        ...
        if DataRequirement.TICKER not in data:
            print(
                "Error: Ticker data not found in data dictionary for StochasticMeanReversionStrategy."
            )
            return signals

        ticker_data_df = data[DataRequirement.TICKER]

        if ticker_data_df.empty or not all(
            col in ticker_data_df.columns for col in [OPEN, HIGH, LOW, CLOSE]
        ):
        ...
        for ticker in tickers:
            try:
                ...
                signal = self.stop_loss_manager.check_stop_loss(ticker, current_price)
                ...
                if confirmed_signal != 0 and (
                    ...
                    # Update entry prices both in strategy and stop loss manager
    ...
    def _get_aggressive_params_for_ticker(self, ticker: str) -> Dict:
        ...
        aggressive_params["overbought"] = (
            70.0  # Less extreme overbought threshold (was 80)
        )
        aggressive_params["signal_threshold"] = (
            0.2  # Lower threshold for signal generation (was 0.3)
        )
        ...
        if loss_count > 0:
            ...
            aggressive_params["oversold"] = min(
                aggressive_params["oversold"] + (5 * factor), 40
            )
            aggressive_params["overbought"] = max(
                aggressive_params["overbought"] - (5 * factor), 60
            )
            aggressive_params["signal_threshold"] = max(
                aggressive_params["signal_threshold"] - (0.05 * factor), 0.1
            )
    ...
    def _update_profit_tracking(self, ticker: str, current_price: float) -> None:
        ...
        if (
            ...
            or current_profit_pct > self.max_profit_seen[ticker]
        ):
    ...
    def _confirm_signal(self, ticker: str, current_signal: float) -> float:
        ...
        if abs(current_signal) < 0.3:
            ...
            if (
                ...
                and self.portfolio.holdings[ticker] != 0
            ):
    ...
    def _check_trailing_profit(
        ...
        if (
            ...
            or ticker not in self.max_profit_seen
        ):
            return None

        # Get trigger and distance parameters
        trigger_pct = self.parameters.get(
            "trailing_profit_trigger", 0.1
        )  # 10% profit to activate
        distance_pct = self.parameters.get(
            "trailing_profit_distance", 0.05
        )  # 5% pullback to exit
    ...
    def _check_profit_target(
        ...
        position = self.portfolio.holdings[ticker]
        is_long = position > 0
        profit_target_pct = self.parameters.get(
            "profit_target_pct", 0.25
        )  # Higher profit target (was 0.15)
    ...
    def _handle_position_exit(self, ticker: str, is_profit: bool) -> None:
        ...
        # Update consecutive loss count in stop loss manager
        self.stop_loss_manager.update_consecutive_losses(
            ticker, self.consecutive_losses.get(ticker, 0)
        )
    ...
    def apply_risk_management(self, signals: Dict[str, float]) -> Dict[str, float]:
        ...
        for ticker, signal in signals.items():
            ...
            if ticker in current_positions and abs(current_positions[ticker]) > 0:
                ...
                if (
                    self.parameters.get("use_martingale", True)
                    and ticker in self.consecutive_losses
                ):
                    loss_count = self.consecutive_losses[ticker]
                    # Amplify signal after losses (more aggressive entry)
                    if loss_count > 0:
                        factor = 1.0 + (
                            loss_count * self.parameters.get("martingale_factor", 0.25)
                        )
                        signal = (
                            signal * factor if abs(signal * factor) <= 1.0 else signal
                        )
    ...
    def get_default_parameters(cls) -> Dict:
        ...
        return {
            ...
            "max_capital_per_position": 0.90,  # 40% per position (was 25%)
            "commission": 0.0,
