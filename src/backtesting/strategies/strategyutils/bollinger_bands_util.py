...
def calculate_bollinger_bands(
    prices: pd.Series, window: int, num_std: float, use_ema: bool = False
...
def generate_bollinger_bands_signal_for_ticker(
    prices: pd.Series, params: Dict
) -> float:
    """
    ...
    if percent_b < 0.3:
        ...
        return (
            0.5 + 0.5 * distance_from_middle
        )  # Moderate to strong buy signal (0.5-1.0)
    elif percent_b > 0.7:
        # Near upper band but not above it - strength based on distance from middle
        return (
            -0.5 - 0.5 * distance_from_middle
        )  # Moderate to strong sell signal (-0.5 to -1.0)
    else:
