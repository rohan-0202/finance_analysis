...
def plot_macd(
    ...
    ax1.set_title(f"{ticker_symbol} Price and MACD ({days} days)", fontsize=14)
    ...
    ax2.plot(
        macd_data.index, macd_data["macd"], color="blue", linewidth=1.5, label="MACD"
    )
    ax2.plot(
        macd_data.index, macd_data["signal"], color="red", linewidth=1.5, label="Signal"
    )
    ...
    # Add info text with periods used
